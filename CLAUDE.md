...
- **jenkins_failures.py** - Queries Jenkins for job failures and downloads build logs
...
# Jenkins failures
podman run --rm --env-file .env quay.io/cverna/coreos-agent-tools jenkins_failures.py list <job-name> --last 5
podman run --rm --env-file .env quay.io/cverna/coreos-agent-tools jenkins_failures.py logs <job-name> <build-number>
...
| `JENKINS_URL` | jenkins_failures.py | Jenkins server URL |
| `JENKINS_USER` | jenkins_failures.py | Jenkins username |
| `JENKINS_API_TOKEN` | jenkins_failures.py | Jenkins API token |
...
### Rate Limiting
Both `jenkins_failures.py` and `process_rhcos_cves.py` implement rate limiting (2 req/sec) with exponential backoff retry logic via a shared `throttled_request()` pattern.
